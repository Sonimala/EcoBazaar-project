...
import { Search, Leaf, ArrowRight, ShoppingBag, Heart, X, Eye, Trophy, BadgeCheck, LayoutDashboard, BarChart3, ShieldCheck, Zap, TrendingUp, Settings, Tag, Clock, Coins, User, Utensils, Shirt, Smartphone, Home as HomeIcon, Flower2, Gamepad2, Briefcase, Dog, Sparkles } from 'lucide-react';
import { getProducts, trackActivity } from '../services/productService';
...
const Products: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      // Set a random product as offer or the one with highest points
      const offer = [...data].sort((a, b) => b.pointsValue - a.pointsValue)[0];
      setOfferProduct(offer);
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'Personal Care': return <User className="w-4 h-4" />;
      ...
      case 'Garden': return <Flower2 className="w-4 h-4" />;
      case 'Toys': return <Gamepad2 className="w-4 h-4" />;
      case 'Office': return <Briefcase className="w-4 h-4" />;
      case 'Pet Care': return <Dog className="w-4 h-4" />;
  ...
  return (
    <div className="min-h-screen bg-white">
      {/* Trending Marquee */}
      <div className="bg-slate-900 py-3 overflow-hidden whitespace-nowrap border-b border-white/5 relative z-20">
        <div className="flex animate-marquee gap-12 items-center">
          {[1, 2, 3].map((i) => (
            <React.Fragment key={i}>
              <div className="flex items-center gap-3">
                <Zap className="w-4 h-4 text-emerald-400" />
                <span className="text-[10px] font-black text-white uppercase tracking-[0.2em]">Trending: Bamboo Toothbrush (+42%)</span>
              </div>
              <div className="flex items-center gap-3">
                <Leaf className="w-4 h-4 text-emerald-400" />
                <span className="text-[10px] font-black text-white uppercase tracking-[0.2em]">New Arrival: Solar Power Bank</span>
              </div>
              <div className="flex items-center gap-3">
                <TrendingUp className="w-4 h-4 text-emerald-400" />
                <span className="text-[10px] font-black text-white uppercase tracking-[0.2em]">Eco Score Leader: Organic Cotton Tee</span>
              </div>
            </React.Fragment>
          ))}
        </div>
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 mb-16">
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-emerald-50 rounded-full border border-emerald-100 mb-6">
              ...
              <span className="text-[10px] font-black text-emerald-700 uppercase tracking-widest">Curated Sustainability</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tighter leading-tight mb-6">
              Eco Marketplace<span className="text-emerald-600">.</span>
            </h1>
            <p className="text-slate-500 font-medium text-lg leading-relaxed">
              Discover products with verified low environmental impact. Every purchase supports a more sustainable future for our planet.
            </p>
          </div>
          <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto">
            <div className="relative flex-grow lg:w-80">
              ...
              <input 
                ...
                className="w-full pl-12 pr-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none transition-all font-bold text-slate-900"
                value={searchTerm}
      ...
      <div className="flex flex-col md:flex-row gap-10">
        <aside className="w-full md:w-64 shrink-0">
          <div className="sticky top-24 space-y-8">
            ...
            {offerProduct && (
              <div>
                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 px-2">Special Offers</h3>
            ...
            <div>
              ...
              <div className="flex flex-col gap-1">
                {categories.filter(c => c !== 'All').map((cat) => (
                  <button
                    key={cat as string}
                    onClick={() => {setSelectedCategory(cat as string); setShowLeaderboard(false);}}
                  ...
                  >
                    <div className={`${selectedCategory === cat && !showLeaderboard ? 'text-white' : 'text-slate-400 group-hover:text-emerald-500'}`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-10">
              {filteredProducts.map(product => (
                <Link 
                  ...
                  key={product.id}
                  className="group bg-white rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 overflow-hidden flex flex-col relative"
                >
                  <div className="relative h-64 overflow-hidden">
                    ...
                    <div className="absolute top-6 left-6 z-30 flex flex-col gap-2">
                      <div className={`px-3 py-1.5 rounded-full border font-black text-[10px] uppercase tracking-widest shadow-xl backdrop-blur-md ${getScoreColor(product.carbonScore)}`}>
                        Score: {product.carbonScore}
                      </div>
                      <div className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-xl backdrop-blur-md border border-white/20 ${getEmissionColor(product.emissionLevel)}`}>
                    ...
                    <div className="absolute top-6 right-6 z-30 flex flex-col gap-2">
                      ...
                      >
                        <Heart className={`w-5 h-5 ${user?.likedProducts?.includes(product.id) ? 'fill-current' : ''}`} />
                      </button>
                    </div>

                    {/* Quick View Overlay */}
                    <div className="absolute inset-0 bg-slate-900/0 group-hover:bg-slate-900/40 transition-all duration-500 flex items-center justify-center opacity-0 group-hover:opacity-100 z-20">
                      <button
                        onClick={(e) => {
                          e.preventDefault();
                          e.stopPropagation();
                        ...
                        className="bg-white text-slate-900 px-8 py-3 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center gap-2 transform translate-y-8 group-hover:translate-y-0 transition-all duration-500 shadow-2xl"
                      >
                  ...
                  <div className="p-8 flex-grow flex flex-col">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <p className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.2em] mb-1">{product.brand}</p>
                        <h3 className="text-xl font-black text-slate-900 leading-tight group-hover:text-emerald-600 transition-colors">{product.name}</h3>
                      </div>
                      <div className="text-right">
                        <span className="text-2xl font-black text-slate-900 block">₹{Math.round(product.price * 83 * 0.85).toLocaleString()}</span>
                        <div className="flex items-center gap-1 mt-1 justify-end">
                          <span className="text-[10px] text-slate-400 line-through font-bold">₹{Math.round(product.price * 83).toLocaleString()}</span>
                          <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">15% OFF</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 mb-6">
                      <div className="flex items-center gap-1.5 px-3 py-1 bg-slate-50 rounded-full border border-slate-100">
                        ...
                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-tighter">{product.expectedLifespan}</span>
                      </div>
                      <div className="flex items-center gap-1.5 px-3 py-1 bg-blue-50 rounded-full border border-blue-100">
                        <Eye className="w-3.5 h-3.5 text-blue-500" />
                        <span className="text-[10px] font-black text-blue-700 uppercase tracking-widest">{product.viewCount.toLocaleString()}</span>
                      </div>
                    </div>
                    ...
                    <div className="mt-auto flex items-center gap-3">
                      {user?.cart?.some(item => item.productId === product.id) ? (
                        <button 
                          ...
                          className="flex-grow flex items-center justify-center gap-2 bg-red-50 text-red-700 py-4 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-red-600 hover:text-white transition-all border border-red-100"
                        >
                        ...
                        <button 
                          ...
                          className="flex-grow flex items-center justify-center gap-2 bg-slate-900 text-white py-4 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-emerald-600 transition-all shadow-xl shadow-slate-900/10"
                        >
                          <ShoppingBag className="w-4 h-4" /> Add to Cart
                        </button>
                      )}
                      <div className="p-4 bg-slate-50 rounded-2xl text-slate-400 group-hover:bg-emerald-50 group-hover:text-emerald-600 transition-all border border-slate-100">
          ...
          <div className="relative bg-white w-full max-w-4xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col md:flex-row max-h-[90vh]">
            ...
            <div className="md:w-1/2 bg-slate-50 flex items-center justify-center p-8">
              <img 
                ...
                referrerPolicy="no-referrer"
                onError={(e) => {
                  (e.target as HTMLImageElement).src = `https://picsum.photos/seed/${quickViewProduct.name}/600/600`;
                }}
              />
            </div>
            ...
            <div className="md:w-1/2 p-8 md:p-12 overflow-y-auto">
              <div className="mb-6">
                ...
                <div className="flex items-center gap-4 mb-6">
                  <div className="flex flex-col">
                    <span className="text-2xl font-black text-slate-900">₹{Math.round(quickViewProduct.price * 83 * 0.85).toLocaleString()}</span>
                    <span className="text-[10px] text-slate-400 line-through font-bold">₹{Math.round(quickViewProduct.price * 83).toLocaleString()}</span>
                  </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <button
                  onClick={async () => {
                    const points = await toggleLikeProduct(quickViewProduct.id);
                    if (points > 0) {
                      setLikedPopupData({ product: quickViewProduct, points });
                    }
                  }}
